...
public class mmcJFrame extends javax.swing.JFrame {
    ...
    private List<File> assembleSrcFile(){
        ...
        list.forEach((s) -> {
            listFiles.add(new File(jTextField1.getText() + "/" + s));
        });
        return listFiles;
    }
    private void addMaxFileNumCache() {
        ...
        File[] target_files = target_file.listFiles();
        List<String> target_list = Lists.newArrayList();
        for (File f : target_files) {
            target_list.add(f.getName());
        }
        // sort
        Collections.sort(target_list);

        // get name
        String filename = target_list.get(target_list.size() - 1);
        filename = FilenameUtils.getBaseName(filename);
        filename = filename.substring(filename.length() - 4, filename.length());
        final int mx = Integer.parseInt(filename);
